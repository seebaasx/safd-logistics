...
const UniformCard = ({ uniform, isAdmin, onDelete, onSelect, onEdit }) => (
  <div onClick={() => onSelect(uniform)} className="group relative bg-zinc-900 rounded-[2.5rem] overflow-hidden border border-white/5 cursor-pointer hover:border-red-600/50 transition-all duration-500">
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className={`relative bg-[#111] w-full max-w-md p-10 rounded-3xl border ${error ? 'border-red-600' : 'border-[#d4af37]/30'} shadow-2xl`}>
        <div className="text-center mb-8">
          ...
          <h2 className="text-[#d4af37] text-xl font-black italic uppercase tracking-widest">SISTEMA LOGÍSTICO</h2>
        </div>
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 text-white">
      ...
      <div className="relative bg-[#0d0d0d] w-full max-w-5xl p-10 rounded-[2.5rem] border border-[#d4af37]/40 shadow-2xl max-h-[90vh] overflow-y-auto text-left">
        <div className="flex justify-between items-center mb-10 pb-6 border-b border-white/5">
          <h2 className="text-[#d4af37] text-2xl font-black italic uppercase tracking-widest">REGISTRAR NUEVO UNIFORME</h2>
          <X onClick={onClose} className="text-zinc-600 hover:text-white cursor-pointer transition-all" />
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          <div className="space-y-6">
            <input placeholder="Nombre" className="w-full bg-[#161616] p-4 rounded-xl border border-white/5 focus:border-[#d4af37] outline-none text-white" onChange={e => setFormData({...formData, name: e.target.value})} />
            <input placeholder="Portada URL (1080x1080)" className="w-full bg-[#161616] p-4 rounded-xl border border-white/5 focus:border-[#d4af37] outline-none text-white" onChange={e => setFormData({...formData, portada: e.target.value})} />
            ...
            <textarea placeholder="URLs Galería (coma)" className="w-full bg-[#161616] p-4 rounded-xl border border-white/5 text-white h-24 text-[10px]" onChange={e => setFormData({...formData, imageUrls: e.target.value})}></textarea>
          </div>
        ...
        <div className="flex gap-4 mt-12">
          <button onClick={() => onSave(formData)} className="flex-1 bg-red-700 py-5 rounded-2xl font-black uppercase text-white hover:bg-red-600 transition shadow-xl">ARCHIVAR EN ALMACÉN</button>
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 text-white text-left">
      ...
      <div className="relative bg-[#0d0d0d] w-full max-w-5xl p-10 rounded-[2.5rem] border border-red-600/40 shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <button onClick={() => onSave(formData)} className="w-full bg-red-700 py-5 rounded-2xl font-black uppercase text-white hover:bg-red-600 transition mt-10">GUARDAR CAMBIOS EN ALMACÉN</button>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-black text-white selection:bg-red-700/50 font-sans tracking-tight">
      <nav className="fixed w-full z-[60] bg-black/60 backdrop-blur-xl border-b border-white/10 h-20 flex items-center justify-between px-6">
        <div className="flex items-center gap-4 cursor-pointer" onClick={() => {setView('landing'); setSelectedDept(null);}}>
          ...
          <div className="text-left hidden sm:block">
            <h1 className="text-xl font-black italic uppercase leading-none">SAFD LOGISTICS</h1>
        ...
        <div className="animate-in fade-in duration-1000">
          <section className="relative h-screen flex items-center justify-center overflow-hidden">
            ...
            <div className="relative z-20 text-center">
              <img src={LOGO_URL} className="h-48 w-48 mx-auto mb-10 drop-shadow-[0_0_50px_rgba(185,28,28,0.5)]" alt="Logo" />
              <h2 className="text-[#d4af37] text-xs font-black tracking-[1.5em] mb-6 uppercase italic">LOGISTICS • SYSTEM</h2>
      ...
      <footer className="py-40 border-t border-white/5 text-center opacity-30">
        <p className="text-[10px] font-black tracking-[1em]">SAFD LOGISTICS • STATE OF SAN ANDREAS</p>
      </footer>
    </div>
  );
}